...
d3sparql.graphType = function (data, type){
  //Debug data and graph type
  if(d3sparql.debug) { console.log(type); }
  if(d3sparql.debug) { console.log(data); }
  
  //Select visualization based on type parameter
  if (type === "Table") {
...
d3sparql.graph = function(json, config) {
  ...
  //Build relationships dynamically, this sets the node index correctly, and avoids the 'D3.js Force Layout Uncaught TypeError'
...
d3sparql.htmltable = function(json, config) {
  ...
  var head = json.head.vars
  var data = json.results.relation
...
d3sparql.barchart = function(json, config) {
  ...
  scale_y.domain(d3.extent(data, function(d) { return parseInt(d.size) }))
...
//    .attr("transform", "translate(" + opts.margin + "," + 0 + ")")
  ...
  var bar = svg.selectAll(".bar")
    ...
    .attr("transform", "translate(" + opts.margin + "," + 0 + ")")
    .attr("class", "bar")
    .attr("x", function(d) { return scale_x(d.name) })
...
d3sparql.piechart = function(json, config) {
  ...
  var radius = Math.min(opts.width, opts.height) / 2.5 - opts.margin
  ...
  var svg = d3sparql.select(opts.selector, "piechart").append("svg")
    ...
    .attr("height", opts.height)
    .append("g")
    .attr("transform", "translate(" + opts.width / 2.5 + "," + opts.height / 2 + ")")
 ...
 */       
  //(json.head && json.results) ? d3sparql.graph(json, config) : json
...
d3sparql.bubble = function(json, config) {
    ...
    var max_amount = d3.max(data, function (d) {
        return parseInt(d.size, 15);
    });
    
    radius_scale = d3.scale.pow().exponent(0.5).domain([0, max_amount]).range([2, 95]);
    
    //Add some additional values to dataset
    data.forEach(function (d) {
        var node = {
            id: d.name,
            radius: radius_scale(parseInt(d.size, 10)),
            name: d.name,
            value: d.size,
            x: Math.random() * 900,
            y: Math.random() * 800
    ...
    //Set up force graph
    var force = d3.layout.force().nodes(nodes).gravity(0.02).charge(function (d) {
  ...
  //Add g element to contain circle and text
   ...
   //add text to the group    
    ...
    function tick(e) {
         node.each(move_towards_center(e.alpha)).each(collide(.5))
        .attr("transform", function (d) {
    ...
    function collide(alpha) {
        ...
        return function (d) {
            var r = d.radius + max_amount + Math.max(padding, padding),
                nx1 = d.x - r,
                nx2 = d.x + r,
                ny1 = d.y - r,
                ny2 = d.y + r;
...
function packageHierarchy(classes) {
  ...
  classes.results.relation.forEach(function(d) {
    find(d.label, d);
  });
